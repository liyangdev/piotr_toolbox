...
% settable constants
maxSkip   = 1000;  % max value for skip
nStep     = 11;   % number of objects to display in lower panel
repLen    = 1;    % number of seconds to replay on left replay
maxCache  = 2000;  % max cache length (set as high as memory allows)
fps       = 150;  % initial fps for playback (if 0 uses actual fps)
skip0     = 1;   % initial skip (zoom)
seqPad    = 1.5;    % amount of padding around object in seq view
siz0      = 5;   % minimum rect width/height
...
enStr = {'off','on'};
[hFig, panelLeft, panelRt, pMenu, panelObj, panelSeq ] = deal([]);
...
%         ps = 'Position';
        ...
        axsPr=[units {'Parent'}];
        o4 = [1 1 1 1];
        clr = [.1 .1 .1]; % background color
        
        panelProp = [units, {bgColor,clr, 'BorderType','none', 'Parent'}];
        buttonProp = {bgColor,[.7 .7 .7], fontSize,7, 'Position'};
        
        checkButtonProp = {'Style','checkbox', fontSize,8, 'Interruptible','off', ...
            bgColor,clr, fgColor,'w', 'Position'};
        
        txtProp = {'Style','text', fontSize,8, bgColor,clr, fgColor,'w', 'Position'};
        editProp = {'Style','edit', fontSize,8, bgColor,clr, fgColor,'w', 'Position'};
        ...
        pos = [(screenSize(3)-780)/2, (screenSize(4)-580)/2, 780, 580];
        ...
        panelObj.h=uipanel(panelProp{:},hFig);
        panelObj.hObjTp = uic( panelObj.h,'Style','popupmenu',fontSize,8,units{:},...
            'Position',[31 2 100 25],'String',objTypes,'Value',1);
        panelObj.hBtPrv = uic(panelObj.h,buttonProp{:},[5   3 25 25],'CData',icn.rNext{1});
        panelObj.hBtNxt = uic(panelObj.h,buttonProp{:},[132 3 25 25],'CData',icn.rNext{2});
        panelObj.hBtNew = uic(panelObj.h,buttonProp{:},[169 3 25 25],'CData',icn.rNew);
        panelObj.hBtDel = uic(panelObj.h,buttonProp{:},[196 3 25 25],'CData',icn.rDel);
        panelObj.hCbFix = uic(panelObj.h,checkButtonProp{:},[230 9 50 13],'String','Lock');
        panelObj.hStSiz = uic(panelObj.h,txtProp{:},[280 9 80 13],hAlignment,'Center');
        panelObj.hCn = [panelObj.hObjTp panelObj.hBtNew panelObj.hBtDel ...
            panelObj.hBtPrv panelObj.hBtNxt panelObj.hCbFix];
        
        % pSeq: bottom panel containing object sequence
        panelSeq.h=uipanel(panelProp{:},hFig);
        panelSeq.hAx = axes(axsPr{:},panelSeq.h,'Position',o4);
        % set color for selected / unselected condition
        panelSeq.apiRng = selectorRange(panelSeq.h,o4,nStep,[],[.1 .5 1]);
%         panelSeq.apiOcc = selectorRange(panelSeq.h,o4,nStep,[],[.9 .9 .7]);
        panelSeq.apiOcc = selectorRange(panelSeq.h,o4,nStep,[],[.9 .1 .1]);
        panelSeq.apiLck = selectorRange(panelSeq.h,o4,nStep,[],[.5 1 .5]);
        panelSeq.lblRng = uic(panelSeq.h, txtProp{:},o4, 'String','vs', fontSize,7, hAlignment,'Center');
        panelSeq.lblOcc = uic(panelSeq.h, txtProp{:},o4, 'String','oc', fontSize,7, hAlignment,'Center');
        panelSeq.lblLck = uic(panelSeq.h, txtProp{:},o4, 'String','lk', fontSize,7, hAlignment,'Center');
        
        % control panel for left image
        % pLf: left main panel
        panelLeft.h = uipanel(panelProp{:}, hFig);
        panelLeft.hAx = axes(axsPr{:}, hFig);
        icn5 = {icn.pPlay{1},icn.pStep{1},icn.pPause,icn.pStep{2},icn.pPlay{2}};
        for i=1:5
            panelLeft.btn(i) = uic(panelLeft.h, buttonProp{:},o4, 'CData',icn5{i});
        end
        panelLeft.btnRep = uic(panelLeft.h, buttonProp{:},[10 5 25 20], 'CData',icn.pRepeat);
        panelLeft.fpsLbl = uic(panelLeft.h, txtProp{:},o4, hAlignment,'Left', 'String','fps:');
        panelLeft.fpsInd = uic(panelLeft.h, editProp{:},o4, hAlignment,'Center');
        panelLeft.hFrInd = uic(panelLeft.h, editProp{:},o4, hAlignment,'Right');
        panelLeft.hFrNum = uic(panelLeft.h, txtProp{:},o4, hAlignment,'Left');
        panelLeft.hCn = [panelLeft.btn panelLeft.btnRep];
        
        % control panel for right image
        % pRt: right main panel
        panelRt.h = uipanel(panelProp{:}, hFig);
        panelRt.hAx = axes(axsPr{:}, hFig);
        panelRt.btnRep = uic(panelRt.h, buttonProp{:},[10 5 25 20], 'CData',icn.pRepeat);
        panelRt.btnGo  = uic(panelRt.h, buttonProp{:},[350 5 25 20], 'CData',icn.fJump);
        panelRt.hFrInd = uic(panelRt.h, txtProp{:},o4, hAlignment,'Right');
        panelRt.hFrNum = uic(panelRt.h, txtProp{:},o4, hAlignment,'Left');
        panelRt.stSkip = uic(panelRt.h, txtProp{:},[55 8 45 14], hAlignment,'Right');
        panelRt.edSkip = uic(panelRt.h, editProp{:},[100 7 28 16], hAlignment,'Center');
        panelRt.stOffst = uic(panelRt.h, txtProp{:},[140 7 30 14], hAlignment,'Center');
        panelRt.slOffst = uic(panelRt.h, 'Style','slider',bgColor,clr,'Position',[175 5 70 20]);
        setIntSlider( panelRt.slOffst, [1 nStep-1] );
        panelRt.hCn = [panelRt.btnRep panelRt.btnGo panelRt.slOffst];
        
        function figResized( h, e ) %#ok<INUSD>
            % overall size of drawable area (fWxfH)
            pos = get(hFig,'Position'); pad=8; pTopH=30;
            ...
            set( panelObj.h,  'Position', [x        y+fH+70 640*r pTopH] );
            set( panelLeft.hAx, 'Position', [x        y+seqH+32 640*r 480*r] );
            set( panelRt.hAx, 'Position', [x+650*r  y+seqH+32 640*r 480*r] );
            set( panelLeft.h,   'Position', [x        y+seqH+2  640*r 30] );
            set( panelRt.h,   'Position', [x+650*r  y+seqH+2  640*r 30] );
            set( panelSeq.h,  'Position', [x+(fW-seqW)/2+2 y seqW seqH] );
            % postion pSeq contents
            set(panelSeq.hAx,'Position',[0 11 seqW seqH-29]); y=1;
            panelSeq.apiLck.setPos([0 y seqW 10]);
            set(panelSeq.lblLck,'Position',[-13 y 12 10]); y=seqH-18;
            panelSeq.apiOcc.setPos([0 y seqW 10]);
            set(panelSeq.lblOcc,'Position',[-13 y 12 10]); y=seqH-9;
            panelSeq.apiRng.setPos([0 y seqW 10]);
            set(panelSeq.lblRng,'Position',[-13 y 12 10]);
            % postion pLf and pRt contents
            x=640*r-90; set(panelRt.btnGo,'Position',[x+60 5 25 20]); x1=640/3*r-75;
            set(panelLeft.hFrInd,'Position',[x 7 40 16]); set(panelLeft.hFrNum,'Position',[x+40 8 40 14]);
            set(panelRt.hFrInd,'Position',[x-30 8 40 14]); set(panelRt.hFrNum,'Position',[x+10 8 40 14]);
            for i2=1:5, set(panelLeft.btn(i2),'Position',[640/2*r+(i2-3.5)*26+10 5 25 20]); end
            set(panelLeft.fpsLbl,'Position',[x1 8 23 14]); set(panelLeft.fpsInd,'Position',[x1+23 7 38 16]);
    ...
    % api for generating new object
    function api = objMakeApi()
        % variables
        [objId, objStart, objEnd, objInd, seqObjs, lims] = deal([]);
        apiRng = panelSeq.apiRng;
        apiOcc = panelSeq.apiOcc;
        apiLck = panelSeq.apiLck;
        
        % callbacks
        set(panelObj.hBtNew, 'callback', @(h,e) objNew());
        set(panelObj.hBtDel, 'callback', @(h,e) objDel());
        set(panelObj.hBtPrv, 'callback', @(h,e) objToggle(-1));
        set(panelObj.hBtNxt, 'callback', @(h,e) objToggle(+1));
        set(panelObj.hObjTp, 'callback', @(h,e) objSetType());
        set(panelObj.hCbFix, 'callback', @(h,e) objSetFixed());
        ...
        function init()
            ...
            set( panelObj.hObjTp, 'String', objTypes, 'Value',1 );
        end
        
        function prepPlay()
            if (objId > 0)
                A = vbb('setRng', A, objId, objStart+1, objEnd+1);
            end
            set(panelObj.hStSiz, 'String', '');
            set(panelObj.hCn, 'Enable', 'off');
            apiRng.enable(0);
            apiOcc.enable(0);
            apiLck.enable(0);
        end
        ...
        function seqI = prepSeq()
            ...
            isVis = objId>0 && objStart<=lstInd && objEnd>=curInd;
            set(panelObj.hStSiz, 'String', '');
            set(panelObj.hCn, 'Enable', 'on');
            apiRng.enable(isVis);
            apiOcc.enable(isVis);
            apiLck.enable(isVis);
            if(~isVis)
            ...
            objSe = min(objStart, curInd);
            objEe = max(objEnd, lstInd);
            ...
            s = max(floor((objStart-curInd)/skip)+1,1);
            e = min(floor((objEnd-curInd)/skip)+1,n);
            rng = zeros(1,nStep);
            occ = rng;
            lck = rng;
            rng(s:e) = 1;
            ...
            if(objStart<curInd), lk=0; else lk=[]; end; apiRng.setLockLf(lk);
            if(objEnd>lstInd), lk=0; else lk=[]; end; apiRng.setLockRt(lk);
            
            % update other gui controls
            objType=vbb('getVal',A,objId,'lbl');
            set(panelObj.hObjTp,'Value',find(strcmp(objType,objTypes)));
            p=ceil(vbb('getVal',A,objId,'pos',objInd+1));
            set(panelObj.hStSiz,'String',[num2str(p(3)) ' x ' num2str(p(4))]);
        ...
        function hs = drawRects( flag, ind )
            ...
            switch flag
                case {'panelLf','panelRt'}
                    objLists = A.objLists{ind+1};
                    n = length(objLists);
                    if(n>0)
                        [~, ord] = sort([objLists.id] == objId);
                        objLists = objLists(ord);
                    end
                    lockSet = get(panelObj.hCbFix,'Value');
                    playMode = strcmp(get(panelObj.hObjTp,'enable'),'off');
                    for i=1:n, obj=objLists(i); id=obj.id; lbl=A.objLbl(id);
                        if(A.objHide(id)), continue; end
                        if(lockSet && id~=objId && ~playMode), continue; end
                        static=(lockSet && id~=objId) || playMode;
                        hs1=drawRect(obj.pos,obj.posv,lims,lbl,static,id,ind,-1);
                        hs=[hs hs1]; %#ok<AGROW>
                    end
                case 'objSeq'
                    if(objInd==-1)
                    ...
                    for i=1:n
                        obj = seqObjs(i);
                        ind = curInd+skip*(i-1);
                        hs1=drawRect(obj.pos,obj.posv,obj.lims,lbl,0,id,ind,i-1);
                        hs=[hs hs1]; %#ok<AGROW>
                    end
            end
            ...
            function hs = drawRect(pos,posv,lims,lbl,static,id,ind,sid)
                ...
                rectProp = {'lw',1.5, 'color',col, 'ls',ls, 'rotate',0, 'ellipse',0};
                
                [hs,api] = imRectRot('pos',pos, 'lims',lims, rectProp{:});
                ...
                if( any(posv) )
                    rectProp = {'LineWidth',1.5, 'EdgeColor','y', 'LineStyle', ':'};
                    hs = [hs, rectangle('Position', posv, rectProp{:})];
                end
            end
            
            function objSetPos( pos, id, ind, sid )
                if(sid>=0), obj=seqObjs(sid+1); pos=obj.pos0-(obj.pos-pos)/obj.res; end
                if(sid>0), dispApi.setOffset(sid); end
                pos=constrainPos(pos); A=vbb('setVal',A,id,'pos',pos,ind+1);
                if(objId==id), objStart=min(objStart,ind); objEnd=max(objEnd,ind); end
        ...
        function objNew()
            [A, obj] = vbb('emptyObj',A,curInd+1);
            t = get(panelObj.hObjTp, 'Value');
            obj.lbl = objTypes{t};
            if(colType == 0)
                t = obj.id;
            end
            col = rectCols(mod(t-1, size(rectCols,1))+1, :);
            rp = {'lw',1.5, 'color',col, 'ls','-', 'rotate',0, 'ellipse',0};
            pos = dispApi.newRect(lims,rp);
            obj.pos = constrainPos(pos);
            A = vbb('add', A, obj);
            objSelect(obj.id, curInd);
            dispApi.requestUpdate();
        end
        ...
        function objSetLck( rng0, rng1 )
            assert(objId>0);
            [lf, rt] = apiRng.getBnds(rng0~=rng1);
            ...
            s = max(objStart,curInd);
            e = min(objEnd,curInd+skip*(nStepVis()-1));
            if(~rng1(lf) || s==e)
                dispApi.requestUpdate();
                return;
            end
            % interpolate intermediate positions
            obj = vbb('get', A, objId, s+1, e+1);
            pos = [obj.pos];
            [n, k] = size(pos);
            lks = [obj.lock];
            lks([1, end]) = 1;
            lks = find(lks);
            for i=1:k
                pos(:, i) = interp1(lks, pos(lks,i), 1:n, 'cubic');
            end
            
            pos = constrainPos(pos);
            A = vbb('setVal', A, objId, 'pos', pos, s+1, e+1);
            dispApi.requestUpdate();
        end
        ...
        function objSetRng( rng0, rng1 )
            ...
            if(lf1~=lf0), objStart=curInd+(lf1-1)*skip; end
            if(rt1~=rt0), objEnd=curInd+(rt1-1)*skip; end
            if(lf1~=lf0 || rt1~=rt0), objSelect(objId,objInd); end
            dispApi.requestUpdate();
        end
        
        function objSetOcc( range0, range1 )
            assert(objId>0);
            [left, right] = apiRng.getBnds(range0~=range1);
            assert(left > 0);
            
            if (left>1 && range0(left-1)==1)
                left = left-1;
                range1(left) = 1;
            end %extend lf
            
            if (right<nStep && range0(right+1)==1)
                right = right+1;
                range1(right) = 1;
            end %extend rt
        ...
        function objSelect( id, ind )
            ...
            if(id==-1)
                [objId,objStart,objEnd,objInd] = deal(-1);
                return;
            end
            objStart = vbb('getVal', A, id, 'str')-1;
            objEnd = vbb('getVal', A, id, 'end')-1;
            objId = id;
            objInd = ind;
        end
        ...
        function pos = constrainPos( pos )
            posTemp = pos;
            posTemp(:, 3:4) = max(1, posTemp(:,3:4));
            r = max(1, siz0./posTemp(:,3:4));
            dy = (r(:,2)-1).*posTemp(:,4);
            
            posTemp(:, 2) = posTemp(:, 2) - dy/2;
            posTemp(:, 4) = posTemp(:, 4) + dy;
            
            dx = (r(:,1)-1).*posTemp(:,3);
            posTemp(:, 1) = posTemp(:, 1) - dx/2;
            posTemp(:, 3) = posTemp(:, 3) + dx;
            
            start = posTemp(:, 1:2);
            edge = start + posTemp(:, 3:4);
            
            for j=1:2
                start(:,j) = min(max(start(:,j), lims(j)), lims(j+2)-siz0);
            end
            for j=1:2
                edge(:,j) = max(min(edge(:,j),lims(j+2)),start(:,j)+siz0);
            end
            pos = [start, edge-start];
        end
        ...
        function A1 = annForSave()
            if(objId==-1)
                ...
                A1 = vbb('setRng',A,objId,objStart+1,objEnd+1);
            end
        end
    ...
    function api = dispMakeApi()
        ...
        % callbacks
        set( panelRt.slOffst, 'Callback', @(h,e) setOffset() );
        set( panelRt.edSkip,  'Callback', @(h,e) setSkip() );
        set( panelLeft.fpsInd,  'Callback', @(h,e) setFps() );
        set( panelLeft.btnRep,  'Callback', @(h,e) setPlay('replayLf') );
        set( panelRt.btnRep,  'Callback', @(h,e) setPlay('replayRt') );
        set( panelRt.btnGo,   'Callback', @(h,e) setFrame('go') );
        set( panelLeft.hFrInd,  'Callback', @(h,e) setFrame() );
        set( panelLeft.btn(1),  'Callback', @(h,e) setPlay(-inf) );
        set( panelLeft.btn(2),  'Callback', @(h,e) setFrame('-') );
        set( panelLeft.btn(3),  'Callback', @(h,e) setPlay(0) );
        set( panelLeft.btn(4),  'Callback', @(h,e) setFrame('+') );
        set( panelLeft.btn(5),  'Callback', @(h,e) setPlay(+inf) );
        ...
        function init( sr1 )
            ...
            hs = [panelLeft.hAx panelRt.hAx panelSeq.hAx];
            for h=hs; cla(h); set(h,'XTick',[],'YTick',[]); end
            set([panelLeft.hCn panelRt.hCn],'Enable',enStr{(nFrame>0)+1});
            set([panelLeft.hFrInd panelRt.hFrInd],'String','0');
            set([panelLeft.hFrNum panelRt.hFrNum],'String',['/' int2str(nFrame)]);
            looping=0;
            requestUpdate();
        end
        ...
        function pos = newRect( lims, rp )
            ...
            hs=[panelObj.hCn panelLeft.hCn panelRt.hCn pMenu.hCn];
            en=get(hs,'Enable'); set(hs,'Enable','off');
            [hR,api]=imRectRot('hParent',panelLeft.hAx,'lims',lims,rp{:});
        ...
        function setSkip( skip1 )
            if(nargin==0), skip1=round(str2double(get(panelRt.edSkip,'String'))); end
            if(~isnan(skip1)), skip=max(1,min(skip1,maxSkip)); end
            if(nFrame>0), skip=min(skip,floor(nFrameRt()/offset)); end
            set( panelRt.stSkip, 'String', 'zoom: 1 / ');
            set( panelRt.edSkip, 'String', int2str(skip) );
            set( panelRt.stOffst,'String', ['+' int2str(offset*skip)] );
            setPlay(0);
        end
        ...
        function setOffset( offset1 )
            if( nargin==1 )
                ...
                offset = round(get(panelRt.slOffst, 'Value'));
            end
            if(nFrame>0)
                offset=min(offset,floor(nFrameRt()/skip));
            end
            set( panelRt.slOffst, 'Value', offset );
            set( panelRt.stOffst, 'String', ['+' int2str(offset*skip)] );
            setPlay(0);
        end
        
        function setFrame( f )
            if(nargin==0), f=round(str2double(get(panelLeft.hFrInd,'String'))); end
            if(strcmp(f, '-'))
                f = curInd-skip+1;
            end
            if(strcmp(f, '+'))
            ...
            if(strcmp(f, 'go'))
                f = curInd+skip*offset+1;
            end
            if(~isnan(f))
                curInd=max(0,min(f-1,nFrame-skip*offset-1));
            end
            set(panelLeft.hFrInd, 'String', int2str(curInd+1));
            setPlay(0);
        end
        ...
        function setPlay( type )
            switch type
                ...
                otherwise
                    ...
                    replay = 0;
                    if (nPlay < 0)
                        nPlay = max(nPlay, -curInd);
                    end
                    if (nPlay > 0)
                        nPlay = min(nPlay, nFrameRt-offset*skip);
                    end
            end
            t = clock();
            t = t(6)+60*t(5);
        ...
        function setFps( fps )
            if (nargin==0)
                fps = round(str2double(get(panelLeft.fpsInd,'String')));
            end
            if (isnan(fps))
                fps = info.fps;
            else
                fps = max(1, min(fps, 99999));
            end
            set(panelLeft.fpsInd,'String',int2str(fps));
            info.fps = fps;
            setPlay(0);
        end
    ...
    function api = filesMakeApi()
        ...
        function saveAnn()
            ...
            tSave1 = tSave;
            if (exist(fAnn,'file'))
                copyfile(fAnn, vbb('vbbName',fAnn,1));
            end
            vbb('vbbSave', A1, fAnn);
            objApi.annSaved();
        end
